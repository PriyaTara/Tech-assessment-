...
public class RegistrationDesk extends UtilityClass {
	...
	public void user_has_to_pick_any_location_and_click_on_Login_and_verify_user_redirected_to_dashboard() throws IOException {
	...
	public void user_has_to_click_patient_register_menu_and_fill_all_details_in_demographics_and_contact_info() throws IOException {
	    
		clickElement(d.getRegister());
		p = new PatientDetailsPOJO();
		passText("Sathya", p.getGivenName());
		passText("S", p.getFamilyName());
		clickElement(p.getNextButton());
		selectValueInDd(p.getGenderDropDown(), "F");
		clickElement(p.getNextButton());
		passText("23", p.getBirthDate());
		selectValueInDd(p.getBirthMonth(), "10");
		passText("2000", p.getBirthYear());
		clickElement(p.getNextButton());
		passText("New Nagar", p.getAddressOne());
		passText("Tambaram East", p.getAddressTwo());
		passText("Chennai", p.getCity());
		passText("Tamil Nadu", p.getState());
		passText("India", p.getCountry());
		passText("600059", p.getPostalCode());
		clickElement(p.getNextButton());
		passText("9888902400", p.getPhoneNumber());
		clickElement(p.getNextButton());
		selectUsingIndex(p.getRelationship(), 7);
		passText("Guru", p.getPersonName());
		clickElement(p.getNextButton());
		
		takeScreenshot("02.Patient Details");
		
		Assert.assertEquals("Wrong Patient Name", true, p.getDetails().get(0).getText().contains("Sathya, S"));
		Assert.assertEquals("Wrong Gender", true, p.getDetails().get(1).getText().contains("Female"));
		Assert.assertEquals("Wrong Birthdate", true, p.getDetails().get(2).getText().contains("23, October, 2000"));
		Assert.assertEquals("Wrong Address", true, p.getDetails().get(3).getText().contains("New Nagar, Tambaram East, Chennai, Tamil Nadu, India, 600059"));
		Assert.assertEquals("Wrong PhNo", true, p.getDetails().get(4).getText().contains("9888902400"));
		Assert.assertEquals("Wrong Relatives", true, p.getDetails().get(5).getText().contains("Guru - Child"));

		clickElement(p.getSubmit());
				
		String date1String = "2000-10-23";
		String date2String = "2024-04-27";// Convert the date strings to LocalDate objects
	...
	public void user_has_to_confirm_start_visit_then_attaching_problems_then_verifying_successfully_attached_message_and_ending_visit() throws Exception {
		...
		Thread.sleep(15000);
		passText("I have fever", v.getCaption());
	...
	public void user_has_to_visit_again_for_body_bmi_calculation() throws IOException {
	...
	public void user_has_to_add_past_visit_and_delete_the_records() throws AWTException, IOException {
		...
		passText("I cannot come", ad.getReason()); 
